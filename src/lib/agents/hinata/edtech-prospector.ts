...
export const EDTECH_FILTERS = {
  ...
  jobTitles: [
    ...
    'L&D Director', 'Head of Content', 'Head of Marketing',
    'Digital Marketing Manager', 'Head of Product',
  ],
  locations:      ['United Kingdom', 'France', 'Canada', 'Belgium', 'Switzerland'],
  employeeRanges: ['1,50', '50,200'],
} as const;

export interface EdtechProspect {
  id:        string;
  name:      string;
  email:     string;
  company:   string;
...
export async function findEdtechProspects(
  campaignId: string,
  page = 1,
): Promise<{ saved: number; skipped: number; prospects: EdtechProspect[] }> {
  const res = await fetch('https://api.apollo.io/v1/mixed_people/search', {
    ...
    body: JSON.stringify({
      page,
      per_page:                          25,
      person_titles:                     EDTECH_FILTERS.jobTitles,
      organization_industry_tag_ids:     EDTECH_FILTERS.industries,
      person_locations:                  EDTECH_FILTERS.locations,
      organization_num_employees_ranges: EDTECH_FILTERS.employeeRanges,
  ...
  for (const p of people) {
    ...
    saved.push(record as unknown as EdtechProspect);
  }

  return { saved: saved.length, skipped, prospects: saved };
}

export async function getEdtechProspects(limit = 50): Promise<EdtechProspect[]> {
