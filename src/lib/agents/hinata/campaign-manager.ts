...
import { findEdtechProspects,
         getEdtechProspects }   from './edtech-prospector';
...
export async function getOrCreateEdtechCampaign(): Promise<string> {
  const name = `HINATA — EdTech Formation — ${new Date().toISOString().slice(0, 7)}`;
  ...
  const { data, error } = await supabase
    .from('campaigns')
    .insert({ name, status: 'active', target_count: 200, agent_name: 'HINATA' })
    .select('id')
    .single();
...
export async function runEdtechCampaignCycle(): Promise<{
  campaign_id:  string;
  found:        number;
  skipped:      number;
  sample_email: { subject: string; html: string } | null;
}> {
  const campaignId = await getOrCreateEdtechCampaign();

  const { saved, skipped, prospects } = await findEdtechProspects(campaignId);
  ...
  void supabase.from('alerts').insert({
    agent_name: 'HINATA',
    level:      'INFO',
    message:    `Cycle EdTech : +${saved} prospects trouvés, ${skipped} ignorés (campaign ${campaignId})`,
  });
  ...
  if (saved > 0) {
    void fetch(process.env.SLACK_WEBHOOK_PROSPECTS!, {
      ...
      body:    JSON.stringify({ text: `🎓 HINATA — ${saved} nouveaux prospects EdTech & Formation\nCampagne : ${campaignId}` }),
    });
  }
...
export async function getEdtechCampaignStats(): Promise<CampaignStats[]> {
  const { data, error } = await supabase
    .from('campaigns')
    .select('id, name, created_at')
    .eq('agent_name', 'HINATA')
