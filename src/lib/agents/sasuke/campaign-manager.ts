...
import { findSaasProspects,
         getSaasProspects }      from './saas-prospector';
...
export async function getOrCreateSaasCampaign(): Promise<string> {
  const name = `SASUKE — SaaS B2B — ${new Date().toISOString().slice(0, 7)}`;
...
export async function runSaasCampaignCycle(): Promise<{
  campaign_id:  string;
  found:        number;
  skipped:      number;
  sample_email: { subject: string; html: string } | null;
}> {
  const campaignId = await getOrCreateSaasCampaign();

  // 1. Find new SaaS prospects
  const { saved, skipped, prospects } = await findSaasProspects(campaignId);

  // 2. Generate a sample outreach for the first new prospect
  let sampleEmail = null;
  if (prospects.length > 0) {
    try {
      sampleEmail = await writeOutreach(prospects[0], 'initial');
    } catch { /* non-blocking */ }
  }
  ...
  void supabase.from('alerts').insert({
    agent_name: 'SASUKE',
    level:      'INFO',
    message:    `Cycle SaaS : +${saved} prospects trouvés, ${skipped} ignorés (campaign ${campaignId})`,
  });
  ...
  if (saved > 0) {
    void fetch(process.env.SLACK_WEBHOOK_PROSPECTS!, {
      ...
      body:    JSON.stringify({ text: `🎯 SASUKE — ${saved} nouveaux prospects SaaS B2B\nCampagne : ${campaignId}` }),
    });
  }
