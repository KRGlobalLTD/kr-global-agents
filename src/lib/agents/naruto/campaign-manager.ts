...
import { findEcomProspects,
         getEcomProspects }  from './ecom-prospector';
...
export async function getOrCreateEcomCampaign(): Promise<string> {
  const name = `NARUTO — E-Commerce — ${new Date().toISOString().slice(0, 7)}`;
  ...
  const { data, error } = await supabase
    .from('campaigns')
    .insert({ name, status: 'active', target_count: 200, agent_name: 'NARUTO' })
    .select('id')
    .single();
...
export async function runEcomCampaignCycle(): Promise<{
  campaign_id:  string;
  found:        number;
  skipped:      number;
  sample_email: { subject: string; html: string } | null;
}> {
  const campaignId = await getOrCreateEcomCampaign();

  const { saved, skipped, prospects } = await findEcomProspects(campaignId);
  ...
  void supabase.from('alerts').insert({
    agent_name: 'NARUTO',
    level:      'INFO',
    message:    `Cycle E-Commerce : +${saved} prospects trouvés, ${skipped} ignorés (campaign ${campaignId})`,
  });
  ...
  if (saved > 0) {
    void fetch(process.env.SLACK_WEBHOOK_PROSPECTS!, {
      ...
      body:    JSON.stringify({ text: `🛒 NARUTO — ${saved} nouveaux prospects e-commerce\nCampagne : ${campaignId}` }),
    });
  }
...
export async function getEcomCampaignStats(): Promise<CampaignStats[]> {
  const { data, error } = await supabase
    .from('campaigns')
    .select('id, name, created_at')
    .eq('agent_name', 'NARUTO')
