...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    switch (action) {
      ...
      case 'run_campaign': {
        const result = await runEdtechCampaignCycle();
        return NextResponse.json({ agent_name: 'HINATA', ...result });
      }

      case 'find_prospects': {
        const campaignId = body['campaign_id'] as string | undefined;
        if (!campaignId) return NextResponse.json({ error: 'campaign_id requis' }, { status: 400 });
        const result = await findEdtechProspects(campaignId, (body['page'] as number) ?? 1);
        return NextResponse.json({ agent_name: 'HINATA', ...result });
      }
      ...
      case 'write_outreach': {
        ...
        return NextResponse.json({ agent_name: 'HINATA', email });
      }
      ...
      case 'generate_content': {
        ...
        return NextResponse.json({ agent_name: 'HINATA', content });
      }

      case 'content_batch': {
        const count    = (body['count'] as number) ?? 3;
        const contents = await generateEdtechContentBatch(count);
        return NextResponse.json({ agent_name: 'HINATA', contents, count: contents.length });
      }

      case 'get_prospects': {
        const prospects = await getEdtechProspects((body['limit'] as number) ?? 50);
        return NextResponse.json({ agent_name: 'HINATA', prospects, count: prospects.length });
      }

      case 'get_stats': {
        const stats = await getEdtechCampaignStats();
        return NextResponse.json({ agent_name: 'HINATA', stats });
      }
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    if (type === 'prospects') {
      ...
      const prospects = await getEdtechProspects(limit);
      return NextResponse.json({ agent_name: 'HINATA', prospects, count: prospects.length });
    }
